...
from . import periodic_activations as t2v_act
...
class ResNet(nn.Module):
    ...
    class Head(nn.Module):
        ...
        def __init__(
            ...
            *,
            ...
            # if t2v_activation == "sin":
            #     self.t2v_layer = SineActivation(1, t2v_hidden_emb)
        ...
        #def forward(self, x: Tensor, t: Tensor) -> Tensor:
            ...
            # t2v_output = self.t2v_layer(t)
            # compined_output = torch.concat([x, t2v_output])

            # x = self.linear(compined_output)
            x = self.linear(x)
            return x
    ...
    def __init__(
        ...
        *,
        ...
        self.head = ResNet.Head(
            ...
            normalization=normalization,
            activation=activation,
            # t2v_activation=t2v_activation,
            # t2v_hidden_emb=t2v_hidden_emb,
        )

        # self.numerical_features = numerical_features
        # self.time_features = time_features
        # self.t2v_activation = t2v_activation
        # self.t2v_hidden_emb = t2v_hidden_emb
    ...
    def forward(self, x) -> Tensor:
        ...
        #         _t_features.append(x[i])
            
        #     _t_features = torch.stack(_t_features, axis=1)[:, :, 0]

        #     for j in self.numerical_features:
        #         _num_features.append(x[j])
            
        #     _num_features = torch.stack(_num_features, axis=1)[:, :, 0]

        # x = self.first_layer(_num_features)
        # x = self.blocks(x)
        # x = self.head(x, _t_features)
...
class FTTransformer(nn.Module):
    ...
    def __init__(
        self, feature_tokenizer: FeatureTokenizer, transformer: Transformer, t2v_encoder: None,
    ) -> None:
        """
        Note:
        ...
        self.activation = _make_nn_module('ReGLU')
        self.output_layer = nn.Linear(232, 1, True)
    ...
    def make_default(
        ...
        *,
        ...
        for arg_name in [
            ...
            transformer_config[arg_name] = locals()[arg_name]
        return cls._make(n_num_features, cat_cardinalities, transformer_config, t2v_encoder)
    ...
    def forward(self, x_num: Optional[Tensor], x_cat: Optional[Tensor]) -> Tensor:
        ...
        if self.t2v_encoder:
          time_features = x_num[:, -16:]
          time_features = torch.reshape(time_features, (-1, 16, 1))
          t2v_output = self.t2v_encoder(time_features)
          t2v_output = torch.reshape(t2v_output, (x.shape[0], -1))
          #print(f"output shape: {x.shape} --- {t2v_output.shape} --- {time_features.shape}")
          x = torch.cat((x, t2v_output), -1)
          x = x.to('cuda')
          x = self.activation(x)
